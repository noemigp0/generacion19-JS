...
const filterUsers = (strToFilter) => {
  return users.filter((cv) => {
    return (
      cv.name.toLowerCase().match(strToFilter) ||
      cv.email.toLowerCase().match(strToFilter) ||
      cv.username.toLowerCase().match(strToFilter)
    );
  });
};

//escenarios a considerar: ocupar eventlistener
//tomar en cuenta el autocomplete
//change funsiona tambien
//ocupar keyup
//se puede ocuopar match o includes

//solucion de jorge

const filterUserJ = (strToFilter) => {
  //sentencias
  strToFilter = strToFilter.toLowerCase();
  let filteredUsers = users.filter((user, index, arr) => {
    if (
...
let inputSearch = document.getElementById("filter_user");
inputSearch.addEventListener("keyup", () => {
  //obtener el valor a buscar
  let searchTerm = inputSearch.value;
  //filtrar usuarios
  let usersFiltrados = filterUserJ(searchTerm);
  ...
  let template = usersFiltrados.reduce((acc, cv) => {
    return (acc += `
      <a href="#" class="list-group-item list-group-item-action" aria-current="true">
      <div class="d-flex w-100 justify-content-between">
      <h5 class="mb-1">Nombre: ${cv.name}</h5>        
      </div>
      <p class="mb-1"> Phone: ${cv.email}</p>
      <small>Email: ${cv.phone}</small>
      </a>     
    `);
  }, "");
